...
const CSV_HEADERS = [
  'id', 'name', 'manufacturer', 'scale', 'category', 'status',
  'boxNumber', 'year', 'notes', 'boxArtUrl', 'shops', 'addedAt',
] as const;
  ...
  for (const kit of kits) {
    const row = CSV_HEADERS.map((h) => {
      ...
      if (h === 'boxArtUrl') {
        ...
        return escapeCell(url.startsWith('data:') ? '[embedded image]' : url);
      }
...
const STATUS_MAP: Record<string, string> = {
  'in stash': 'Unbuilt', stash: 'Unbuilt', unbuilt: 'Unbuilt',
  wishlist: 'Wishlist', 'wish list': 'Wishlist',
  'on the go': 'WIP', wip: 'WIP', 'work in progress': 'WIP',
  built: 'Complete', complete: 'Complete', completed: 'Complete', finished: 'Complete',
  'sold/donated': 'Sold/Donated', sold: 'Sold/Donated', donated: 'Sold/Donated',
};
